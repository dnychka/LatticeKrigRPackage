...
### Name: IcosahedronGrid
### Title: Icosahedral multi-resolution grids
### Aliases: IcosahedronGrid IcosahedronFaces
### Keywords: spatial
...
cleanEx()
nameEx("KrigingExampleData")
### * KrigingExampleData

flush(stderr()); flush(stdout())

### Name: KrigingExampleData
### Title: Synthetic data for kriging examples
### Aliases: KrigingExampleData

### ** Examples

## Not run: 
##D data(KrigingExampleData)
##D x<- KrigingExampleData$x
##D y<- KrigingExampleData$y
##D plot(x,y)
##D kFit <- LatticeKrig(x, y)
##D xGrid <- seq(0,1,0.001)
##D lines(xGrid, predict(kFit, xGrid), col='red')
## End(Not run)
...
### Name: LKDiag
### Title: Create a matrix with given entries on the given diagonals
### Aliases: LKDiag
...
### Name: LKrig.MLE
### Title: Functions to search over covariance parameters in the
###   LatticeKrig model.
...
##D   set.seed(124)
##D   par.grid<- list()
##D # create grid of alphas to sum to 1 use a mixture model parameterization
...
###   coefficients.
### Aliases: createLKrigObject LKrig choleskyMemory predict.LKrig
###   predictSE.LKrig print.LKrig summary.LKrig print.LKinfo surface.LKrig
###   predictSurface.LKrig
### Keywords: spatial
...
## Not run: 
##D   data(CO2)
##D   # the Ring geometry will be periodic in the first dimension and rectangular on 
...
### Name: LKrig.basis
### Title: Functions for generating a multi-resolution, compactly supported
###   basis, multi-resolution covariance functions and simulating from
###   these processes.
...
### Name: LKrig.sim
### Title: Functions for simulating a multi resolution process following
###   the Lattice Krig covariance model.
...
##D # here x can be any two column matrix of coordinates this
##D # function returns a vector of predictions
##D   out.fhat<- predict( obj, xnew= x)
##D   
##D # conveniently predict on a 100X100 grid for plotting
##D # use the grid.list arugment to give more control over the grid choice.
##D # output object is the standard list with components x, y, and z
##D # suitable for contour, persp, image, etc.
...
##D # compare the LKinfo object here  to one created implicitly:  obj$LKinfo
...
##D # on a 20X20 grid within the spatial domain ( so about 400) 
...
##D #log lambda and omega =  log(a.wght-4)/2 are useful parameterization ...
...
## Not run: 
##D # Surface plot of estimate
##D   surface(obj0, nx=288, ny=165)
##D   world( add=TRUE)
## End(Not run)  
## Not run: 
##D data(CO2)
##D # estimate lambda ( should be around 0.003)
...
##D  surface( obj0B, col=terrain.colors(256))
##D  world( add=TRUE, col="magenta")
##D   
##D # use chordal distance 
##D LKinfo1<- LKrigSetup( CO2$lon.lat, startingLevel=2 ,nlevel=2,
##D                        a.wght=1.1, alpha=c(1,.25),
##D                        LKGeometry="LKSphere", distance.type="Chordal") 
##D                        
##D  obj0C<-  LatticeKrig( CO2$lon.lat,CO2$y,LKinfo=LKinfo1)
##D  surface( obj0C, col=terrain.colors(256))
##D  world( add=TRUE, col="magenta")
##D 
...
### Title: Specifying non-stationary models
...
## Not run: 
##D 
##D #### Anisotropy varying over space
##D #### First check that the constant model can be reproduced
##D   a.wghtM2<- c( rbind( c(  0,   0, -1.5),
##D                        c(-.5, 4.5,  -.5),
##D                        c(-1.5,  0,    0)
##D                    )
##D                    )
##D                    
##D   a.wghtObject<- list( values=a.wghtM2)
##D   class(a.wghtObject )<- "constantValue"
##D  
##D   LKinfo4<- LKrigSetup(sDomain, NC = 5, 
##D               alpha = c(1,.5, .125), nlevel = 3, 
##D               a.wghtObject =  a.wghtObject, normalize=TRUE )
##D   set.seed(123)            
##D   look<- LKrig.sim( fineGrid, LKinfo4)
##D   image.plot( as.surface( fineGrid, look) )            
##D 
##D ###### non-stationary anisotropy
##D  a.wghtA <- c( rbind( c(    0,   0, -2),
##D                        c( 0, 4.5,  0),
##D                        c(-2,  0,     0)
##D                    )
##D                    )
##D  a.wghtB <- c( rbind( c(  -2,   0,     0),
##D                        c(  0, 4.5,  0),
##D                        c(    0,    0, -2)
##D                    )
##D                    )
##D # Now create multivariate prediction object.
##D   gridList<-  attributes( xPoints)$grid.list
##D   m1<- length(gridList$x)
##D   m2<- length(gridList$y) 
##D   z<- array( NA, c( m1,m2,9))
##D   alpha<- (xPoints[,1] + 1 )/2
##D   alpha<- ifelse( alpha <= 0, 0, alpha)
##D   alpha<- ifelse( alpha >= 1, 1, alpha)
##D # A for loop over the 9 pixels   
##D   for(j in 1:9) {
##D # linear combination of two a.wght matrices
##D # 
##D     zTemp<- a.wghtA[j] * (1-alpha) +  a.wghtB[j]*(alpha)
##D # coerce into an image format    
##D     z[,,j]<- as.surface( xPoints, zTemp)$z
##D   }
##D 
##D   a.wghtObject<- list( x= gridList$x,  y= gridList$y, z=z )
##D   class( a.wghtObject)<- "multivariateSurfaceGrid"
##D 
##D   LKinfo5<- LKrigSetup(sDomain, NC = 25, NC.buffer=0,
##D               alpha = c(1,.5), 
##D               nlevel = 2, 
##D               a.wghtObject =  a.wghtObject )
##D   set.seed(122)  
##D   fineGrid<- make.surface.grid(
##D                  list( x = seq(-1, 1, ,150),
##D                        y = seq(-1, 1, ,180)
##D                        )
##D                                )
##D   look<- LKrig.sim( fineGrid, LKinfo5)
##D   image.plot( as.surface( fineGrid, look), col = terrain.colors(256) )
## End(Not run)
...
###   distances.
### Aliases: findnorm lkdist lkdistcomp lkdistgrid lkdistgridcomp lkdiag
### Keywords: datasets
